...
export default function LoginPage() {
  ...
  // Função para checar role e redirecionar
  ...
  const isLocked = useMemo(() => lockoutUntil !== null && Date.now() < lockoutUntil, [lockoutUntil]);
  ...
  }, [canSubmit, form, isValidForm, login, handleSuccessfulAttempt, handleFailedAttempt, checkUserRoleAndRedirect]);
  ...
  useEffect(() => {
    if (!isLocked && emailInputRef.current) emailInputRef.current.focus();
  }, [isLocked]);
  ...
  const shouldShowEmailError = useMemo(() => touched.email && form.email && !form.email.includes('@'), [touched.email, form.email]);
  const shouldShowPasswordError = useMemo(() => touched.password && form.password.length < 6, [touched.password, form.password]);
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                ...
                autoComplete="email"
              />
              {shouldShowEmailError && <p className="text-red-400 text-xs">Digite um email válido</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              {shouldShowPasswordError && <p className="text-red-400 text-xs">Mínimo 6 caracteres</p>}
            </div>
          </div>
